...
3. Create up to $PLANNER_BATCH new issues — only work that is actionable RIGHT NOW (its dependencies already merged). Before creating each candidate, confirm it is not a duplicate with a TARGETED search — `gh issue list --state all --search "<keywords>"` per candidate — never a full listing. Each issue:

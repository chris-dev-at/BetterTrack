...
5. Keep your context lean — every byte of tool output you produce is re-read on all later turns. Batch independent file reads into one turn. Run tests with a quiet reporter and pipe long output through `tail -60`; while iterating run only the targeted test files, then the full relevant suite once before pushing. Never cat whole large files (read the specific ranges you need) and never re-run a suite you already saw pass.
6. Push the branch task/{{N}} and open a PR: title from the issue, body containing "Closes #{{N}}", a summary of what you did, and the real test output. If you add/move/rename modules, note it in the PR body — the module map regenerates automatically each cycle.

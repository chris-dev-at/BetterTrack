...
- Model-tier delegation ladder (CLAUDE.md is authoritative, MODELUSE.md expands it): **T3 sonnet at `high` effort** (`claude-sonnet-5`, agent `sonnet-builder`) is the **floor** — nothing runs below it — for anything it can handle with zero risk to UX/UI or app stability; **T2 opus** (`claude-opus-5`, agent `opus-engineer`) for security boundaries and subtle engineering; **T1 fable** (`claude-fable-5`, agent `fable-core`) is the top tier and owns the money math (`apps/api/src/domain/**` and `packages/domain/src/**`), the provider/caching/currency keystone, the search-index core, and plan deviations. Fable is **not** retired (ruling 2026-09-02, §16); the owner's Fable-conservation policy is a usage policy — spend it only where T1 genuinely applies, one pass not a loop — and never a licence to write T1 code on Opus. When unsure, take the higher tier. Factory routing is by `diff:easy…diff:max` labels (legacy `tier:*` still resolves).

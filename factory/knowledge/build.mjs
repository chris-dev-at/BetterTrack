...
function build() {
  ...
  // Reverse-edge counts: how many files import each node (blast radius at a glance).
  const importedByCount = new Map();
  for (const e of edges) importedByCount.set(e.to, (importedByCount.get(e.to) || 0) + 1);
  for (const n of nodes) n.importedBy = importedByCount.get(n.path) || 0;
...
function renderMap(graph) {
  ...
  out.push('');
  out.push('Query recipes (repo root; one targeted jq beats several greps):');
  out.push(
    '- which file exports symbol X: `jq -r \'.nodes[]|select(.exports|index("X"))|.path\' factory/knowledge/graph.json`',
  );
  out.push(
    '- who imports file F (blast radius before you change it): `jq -r \'.edges[]|select(.to=="F")|.from\' factory/knowledge/graph.json`',
  );
  out.push(
    '- what file F itself imports: `jq -r \'.nodes[]|select(.path=="F")|.imports[]\' factory/knowledge/graph.json`',
  );
  out.push('');
...
function fileLine(n) {
  ...
  const used = n.importedBy ? ` · used by ${n.importedBy}` : '';
  return `- ${basename(n.path)} — ${n.kind}${ex ? ` — ${ex}${more}` : ''}${used}`;
}

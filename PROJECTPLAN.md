...
- **Login** (`POST /auth/login`, email-or-username + password): argon2id verify; on success rotate session ID, set `last_login_at`, audit-log. Generic error message on failure (no user enumeration). Rate limit: 25/min per IP and 20/h per account; lockout 5 min after 20 consecutive failures (relaxed 2026-06-16 — see §16).
...
- **Rate limits** (Redis): login 25/min/IP · search 60/min/user · general API 4500/15 min/user · admin endpoints 600/15 min (relaxed 2026-06-16 — see §16).
...
| 2026-07-01 | Fable restored; outage fallback reverted (see FableBackExecute.md) | Owner |

...
**Owner-provided items:** LLM API key + spend cap for P12 (or P12 is cut), explicit go/no-go on paranoid mode (P13 — he is on record as unsure), Grafana exposure decision (P2: localhost-only vs admin-proxied), Play-Store listing assets when the Android app ships (ops footnote).
...
| **V5-P13 — Paranoid mode (M, diff:hard, auth; owner-confirm before compose — he is on record as unsure)** | Optional per-account read-secret shown ONCE at enable; reading the account's data requires it in addition to the session; server stores only a hash; loss is irrecoverable **by design**; interplay with sharing, exports, admin, and recovery is settled in a §16-logged design note BEFORE any code | The design note is §16-logged and owner-acked; with the mode on, a valid session without the secret reads nothing sensitive (test); disable requires the secret; recovery paths behave exactly per the design note (tests) |

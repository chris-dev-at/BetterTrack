...
Estimate (pre-re-confirmation): **$350–650, 10–15 factory days, 2 workers.** After v5 the parking lot is empty — the next milestone is planned with the owner from scratch (menu process). Ops footnote: when the Android app ships to the Play Store, `mobile.bettertrack.at` flips from placeholder to store redirect (Chief ops task, not a factory issue).
...
**Parked for V6 planning (owner note, 2026-07-09 — NOT in the locked v5 scope):** competitor market research — a full survey of competing portfolio managers/providers (Parqet, getquin, Portfolio Performance, Sharesight, Snowball Analytics, Delta, Simply Wall St, Finanzfluss Copilot, broker-integrated trackers, … esp. DACH): complete feature inventory + pricing structure per competitor, written down raw (tables) so the owner can turn it into his own report, then a gap/parity/differentiator map of every feature BetterTrack has implemented by then. Research work, not factory code — run via research subagents at v6 planning time. Tracking issue: #389.

...
2. **The issue itself is mis-scoped** — too big for one package, wrong decomposition, acceptance criteria demand something outside the touched area, or the real work belongs in a different module. A re-scoped issue is the fix, not another round on this PR. If issue #{{N}} carries a `## Split seam` section, its author already named the boundary and the two halves — prefer that seam over one you invent, and say so if you deviate.
...
- Case 2 — first create the properly-scoped replacement/follow-up through the guarded helper; NEVER call `gh issue create/edit` directly. When issue #{{N}} has a `## Split seam`, the new issue IS its foundational half: scope it to exactly that half, at the difficulty the seam names, and leave the dependent half with the parent (which will reschedule once the child closes). You get exactly ONE new issue — if the seam implies more than two pieces, take the foundational one and describe the rest in the parent. Write the complete body (Context with verbatim spec quotes if extractable from issue #{{N}}, Scope, Acceptance criteria, Out of scope, then terminal `mf-meta` depends-on/touches) to a temporary file and run:

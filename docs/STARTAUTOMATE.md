...
4. Create up to $PLANNER_BATCH new issues — only work that is actionable RIGHT NOW (its dependencies already merged). Each issue:
   ...
   - Labels: autopilot + exactly one tier label per MODELUSE.md (tier:fable for apps/api/src/domain/** and packages/domain/src/** and the provider/caching/currency core; tier:opus for auth/admin/schema/jobs/realtime/Builder/sharing; tier:sonnet for CRUD/UI/config/docs; unsure → higher tier).

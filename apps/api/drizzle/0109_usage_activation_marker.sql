...
--
-- Reading `usage_events` is also what keeps the backfill honest about paranoid
-- accounts: enable purges every one of their rows and capture is suppressed
-- afterwards, so a paranoid account contributes no source row here and is not
-- back-marked as activated (§6.12 "vaulted/paranoid data never counted").
INSERT INTO "usage_activations" ("user_id", "first_active_at")
SELECT "user_id", min("last_seen_at")
FROM "usage_events"
GROUP BY "user_id"

...
interface ParityScenario {
  ...
  /**
   * A standing custody fee (V5, §16 2026-07-30): a positive EUR magnitude booked
   * through `POST /cash/fee`. The kind is INTERNAL for TWR, so unlike a
   * withdrawal it is not divided back out of the curve — it drags it. That makes
   * this scenario the client/server agreement test for the classification: if the
   * paranoid engine treated a `fee` as external (or dropped it), its vector would
   * diverge from the pinned server one.
   */
  cashFee?: { amountEur: number; dayOffset: number };
  twrPct: number[];
  /**
   * The same scenario with `cashFee` omitted — recorded so the drag is legible in
   * the fixture itself and not only as a passing assertion. Regenerated by the
   * strictly-below test, which computes it live rather than trusting this copy.
   */
  twrPctWithoutTheFee?: number[];
}

const fixture = JSON.parse(readFileSync(FIXTURE_PATH, 'utf8')) as {
  sinceInceptionMax: ParityScenario;
  splitDateCashBuy: ParityScenario;
  internalCashFeeDrag: ParityScenario;
};
...
async function serverPerformanceVector(scenario: ParityScenario): Promise<number[]> {
  ...
  if (scenario.cashFee !== undefined) {
    // Booked through the real route, so the pinned vector reflects the production
    // write path (kind `fee`, negative amount, cent-quantized) and not a fixture
    // hand-inserting a row the service might have shaped differently.
    const fee = await agent
      .post(`/api/v1/portfolios/${pid}/cash/fee`)
      .set(...XRW)
      .send({
        amountEur: scenario.cashFee.amountEur,
        executedAt: tsOffset(scenario.cashFee.dayOffset),
      });
    expect(fee.status, JSON.stringify(fee.body)).toBe(201);
    expect(fee.body.movement.kind).toBe('fee');
  }
...
describe('paranoid client TWR parity golden fixture (issue #899)', () => {
  ...
  it('the server reproduces the internal-fee drag vector byte-for-byte', async () => {
    // The classification anchor for the `fee` kind (V5, §16 2026-07-30). The
    // client consumes this same vector, so a paranoid engine that classified a fee
    // as an external flow — or omitted it — fails right here on the other side.
    const pct = await serverPerformanceVector(fixture.internalCashFeeDrag);
    expect(pct).toHaveLength(fixture.internalCashFeeDrag.twrPct.length);
    pct.forEach((value, index) => {
      expect(value).toBeCloseTo(fixture.internalCashFeeDrag.twrPct[index]!, 12);
    });
  });

  it('the fee vector sits strictly BELOW the same ledger with no fee at all', async () => {
    // Guards the fixture against becoming a tautology: a pinned vector that
    // happened to equal the fee-free one would still pass the byte-for-byte test
    // above while proving nothing about the drag.
    const { cashFee: _omitted, ...withoutFee } = fixture.internalCashFeeDrag;
    const withFeePct = await serverPerformanceVector(fixture.internalCashFeeDrag);
    const noFeePct = await serverPerformanceVector(withoutFee);
    expect(withFeePct).toHaveLength(noFeePct.length);
    // The fee-free vector recorded alongside the pinned one stays honest too.
    noFeePct.forEach((value, index) => {
      expect(value).toBeCloseTo(fixture.internalCashFeeDrag.twrPctWithoutTheFee![index]!, 12);
    });
    const feeDay = fixture.internalCashFeeDrag.cashFee!.dayOffset;
    const depositDay = fixture.internalCashFeeDrag.depositDayOffset!;
    // The curve only parts once the fee lands, and never comes back.
    const feeIndex = feeDay - depositDay;
    expect(withFeePct.at(-1)!).toBeLessThan(noFeePct.at(-1)!);
    for (let i = feeIndex; i < withFeePct.length; i += 1) {
      expect(withFeePct[i]!).toBeLessThan(noFeePct[i]!);
    }
  });
});

 ...
 * its main source, `/cash/tags` seeds the app-owned system tags, and the
...
describe('paranoid capture round trip', () => {
  ...
  it('catches a capture whose OWN reads wrote, and lands the retry carrying their rows', async () => {
     ...
     * `/cash/tags` seeds this account's system tags and the tax-report reads run
     * the tax self-heal, which inserts the year's correction cash movement
     * AFTER `…/cash` snapshotted the ledger; (b) the pass-1
    ...
    // ── Pass 2: converged. Both writes are one-shot (the system tags exist, the

...
export function createAlertService(deps: AlertServiceDeps): AlertService {
  ...
  /**
   * Personal price alerts remain available in paranoid mode for global market
   * assets only. Hold the caller's transition lock while selecting that branch
   * and keep it through metadata/quote reads, persistence, and follower fan-out.
   */
  const withVisibleAssetScope = <T>(
    userId: string,
    action: (includeCustomAssets: boolean) => Promise<T>,
  ): Promise<T> =>
    deps.paranoid.runAllowedWithOptional([], [userId], 'portfolioServer', (normalUserIds) =>
      action(normalUserIds.has(userId)),
    );

  return {
    async list(userId) {
      return withVisibleAssetScope(userId, (includeCustomAssets) =>
        repo.listForUser(userId, { includeCustomAssets }),
      );
    },
    ...
    async update(userId, id, patch) {
      return withVisibleAssetScope(userId, async (includeCustomAssets) => {
        const updated = await repo.update(userId, id, patch, { includeCustomAssets });
    ...
    async rearm(userId, id) {
      return withVisibleAssetScope(userId, async (includeCustomAssets) => {
        const rearmed = await repo.rearm(userId, id, { includeCustomAssets });

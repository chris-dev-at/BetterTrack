...
export const EXPORT_TABLE_CLASSIFICATION: Record<string, TableClassification> = {
  ...
  asset_catalog_watermark: skipped(
    'One instance-wide row holding a single timestamp — the monotonic write watermark behind the catalog search’s Last-Modified (#1709, #1762). No user id, no asset id, nothing to attribute to an account.',
  ),
...
export const PARANOID_TABLE_CLASSIFICATION: Record<string, ParanoidClassification> = {
  ...
  usage_daily: 'server',
  // The catalog-search write watermark (#1709, #1762): ONE instance-wide row,
  // one timestamp column, no user id and no asset id. It is stamped from the
  // touched rows' own creation instants precisely so it carries no identifier —
  ...
  asset_catalog_watermark: 'server',
  app_settings: 'server',

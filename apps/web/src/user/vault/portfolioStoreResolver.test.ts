...
async function resolverFixture(state: 'locked' | 'unlocked' = 'unlocked') {
  ...
  const vault = {
    ...
    // Deliberately low-entropy placeholder: this resolver test only ever hands
    // the fingerprint back through a stubbed `openStoredVault`, so it never has
    // to equal the value `keys.test.ts` derives from the public BIP39 vector.
    // Keeping it obviously fake leaves Gitleaks' generic-API-key detector armed
    // over this file instead of buying silence with an allowlist entry.

...
export function createDriveDataHome(options: DriveDataHomeOptions): DriveDataHome {
  ...
  return {
    ...
    async read(): Promise<DataHomeReadResult> {
      ...
      const downloaded = await driveFetch(
        `${DRIVE_API}/files/${encodeURIComponent(found.file.id)}?alt=media`,
      );
      ...
      const inspected = inspectEnvelope(envelope, found.file);
  ...
  async function upload(
    ...
    let validated: DriveFileResult;
    try {
      validated = validateFile(await uploaded.response.json());
    } catch (cause) {
      return transport({
        code: 'api-failure',
    ...
    if (validated.status === 'failure') return transport(validated.failure);
    if (validated.status === 'corrupt') return validated.result;
    ...
    if (validated.file.version !== outgoing.version) {
      return corrupt(
        envelope,
        validated.file.version,
        'version-mismatch',

...
export function decodeBase64Url(value: string): Uint8Array {
  if (!/^[A-Za-z0-9_-]*$/u.test(value) || value.length % 4 === 1) {
    throw new EndpointKeystoreError('storage-invalid', 'Keystore data is not base64url.');
  }
  ...
  try {
    ...
    if (encodeBase64Url(bytes) !== value) {
      throw new EndpointKeystoreError(
        'storage-invalid',
        'Keystore data is not canonical base64url.',
      );
    }
    return bytes;
  } catch (cause) {
    ...
    throw new EndpointKeystoreError(
      'storage-invalid',
      'Keystore data is not valid base64url.',
      {},
      { cause },
    );
  }

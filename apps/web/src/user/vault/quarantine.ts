...
/** Original opaque bytes retained after a validation failure. */
export interface QuarantinedVaultCandidate {
  ...
  /** Diagnostic only; never decrypted portfolio content. */
  reason: string;
}
...
/** Encrypted-byte-only quarantine sharing the local cache's IndexedDB schema. */
    ...
    async put(candidate) {
      ...
      try {
        ...
        for (const entry of all
          .filter((value) => value.scope === options.scope)

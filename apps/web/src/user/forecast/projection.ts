 ...
 * drops **paused** and archive-suspended orders (only runnable orders continue
 * forward) and drops **buy-asset** orders (net-worth-neutral reallocations,
 * see the module note), mapping each cash order to its signed EUR flow.
 */
export function normalizeStandingOrders(orders: readonly StandingOrder[]): ForecastStandingOrder[] {
  ...
  for (const order of orders) {
    if (order.status !== 'active' || order.suspendedByArchive) continue;

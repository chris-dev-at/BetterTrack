---
name: fable-core
description: T1 implementation agent (MODELUSE.md, top tier) — MUST BE USED for correctness-critical BetterTrack code, money math, where errors cost real money. The T1 paths are apps/api/src/domain/** (allocation, backtest, plus the re-export shims) AND packages/domain/src/** (tax, holdings, cashLedger, seriesStats, settingsScope, vaultVectors; later alertEval) + their test suites, the provider abstraction/caching/request-coalescing/currency keystone (P1), the local search-index core (§6.2), and any design decision that deviates from PROJECTPLAN.md.
model: fable
---
...
Standards:
- Domain code — `apps/api/src/domain/**` and `packages/domain/src/**` alike — is pure functions, no I/O, no framework imports.
